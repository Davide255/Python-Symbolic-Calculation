...
class Radical(object):
    ...
    def semplify(self):
        ...
        if isinstance(self.base, (Integer, int, Fraction)):
        ...
        elif isinstance(self.base, ImaginaryUnit):
            _c = Radical(abs(self.base.coefficient), self.index, self.coefficient)
            return _c * LiteralFraction(Radical(2), 2) * (1 + (I*sign(self.base.coefficient)))
            ...
            if isinstance(self.base.coefficient, (int, Integer)):
                if self.index % 2 == 0 and self.base.coefficient < 0: 
                    self.base = self.base.__new__(type(self.base), [abs(self.base.coefficient), self.base.symbol, self.base.esponent])
    ...
    def __mul__(self, value):
        ...
        elif isinstance(value, (Polinomial, LiteralFraction)):
            return value * self
    ...
    def __truediv__(self, value):
        if isinstance(value, (Integer, int, float, Fraction)):
            ...
            if self.base == value.base and self.index == value.index:
                return LiteralFraction(self.coefficient, value.coefficient)
            elif self.index == value.index:
                return Radical([LiteralFraction(self.coefficient, value.coefficient), LiteralFraction(self.base, value.base), self.index])
            elif self.base == value.base:
                return Radical([LiteralFraction(self.coefficient, value.coefficient), self.base, self.index - value.index])
...
class Unknow(object):
    ...
    def __neg__(self):
        return self.__new__(type(self), [-self.coefficient, self.symbol, self.esponent])
    ...
    def __mul__(self, value):
        ...
        if isinstance(value, (Unknow, Literal, Fraction, LiteralFraction, Radical)):
            ...
            return self.__new__(type(self), [self.coefficient * value, self.symbol, self.esponent])
...
class UnknownMultiplication(object):
    ...
    def __new__(cls, *literals, coefficient_mult=1) -> None:
        ...
        _num = 1
        for l in literals:
            if isinstance(l, (Unknow, Literal)):
                if ref.get(l.symbol): ref[l.symbol] = ref[l.symbol] * l
                else: ref[l.symbol] = l
            elif isinstance(l, Radical):
                ref[str(l)] = l
            else: _num = _num * l
        ...
        for i in self.literals:
            if isinstance(i, (int, Integer)):
                ...
                self.literals[self.literals.index(i)] = Radical(i.base)
            else:
                if self.coefficient: 
                ...
                self.literals[self.literals.index(i)] = i.__new__(type(i), [1, i.symbol, i.esponent])
...
class Polinomial(object):
    ...
    def scompone(self, poly: Polinomial = None):
        ...
        elif not poly: poly = Polinomial(*self.terms.copy(), esponent=self.esponent)
        elif not isinstance(poly, Polinomial): poly = Polinomial(poly)

        if poly.esponent != 1: poly = poly._calculate_pow_()
        ...
        if poly == Trinomial:
            ...
            if not isinstance(a, (Polinomial, Radical)) and (s.coefficient if isinstance(s, (Unknow, Literal)) else s.number) < 0:
                b = Radical(abs(s))
                if not isinstance(b, Radical):
...
'''
            ...
            if uk[0].esponent == 2:
                ...
                if len(uk) == 2 and uk[0].symbol == uk[1].symbol \
                    ...
                    if delta > 0:
                        ...
                        _rad = Radical(abs(delta))*I

                    s1 = LiteralFraction((_rad -b), (2*a)),
                    s2 = LiteralFraction((-_rad -b), (2*a))
    ...
    def create_existence_conditions(self, obj):
        from Core._utils import Condition, ExistenceConditions
...
class ImaginaryUnit(Unknow):
    ...
    def __pow__(self, value):
        ...
        elif isinstance(value, (int, Integer)) and self.esponent*value > 1 and (self.esponent * value) % 2 == 1:
            if (self.esponent * value) % 4 == 1: return self.coefficient**value*I
            else: return -(self.coefficient**value)*I
        else:
            return ImaginaryUnit(coefficient=self.coefficient ** value, esponent = self.esponent * value)
